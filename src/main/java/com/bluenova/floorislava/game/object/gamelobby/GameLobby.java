...
public class GameLobby extends Lobby {
    ...
    public Location gameStartLoc;
    public Location gameEndLoc;
    boolean gameON = true;
    public int lavaHeight = -63 + LAVA_INCREMENT;

    public GameLobby(ArrayList<Player> playerList, Player owner, InviteLobbyManager inviteLobbyManager, GamePlot plot) {
        super(playerList, owner);
        ...
        for (int x_index = 0; x_index < FloorIsLava.getGamePlotDivider().plotSize; x_index++) {
            for (int z_index = 0; z_index < FloorIsLava.getGamePlotDivider().plotSize; z_index++) {
                if (z_index == FloorIsLava.getGamePlotDivider().plotSize - 1 && x_index == FloorIsLava.getGamePlotDivider().plotSize - 1)
                    gp = this;
                FloorIsLava.getWorkloadRunnable().addWorkload(new GenerateGameTerrain(gp, x_copy + x_index, z_copy + z_index, x_paste + x_index, z_paste + z_index));
            }
        }
    }
    ...
    public void startGame() {
        Bukkit.getScheduler().runTaskTimer(FloorIsLava.getInstance(), (task) -> {
            if (countDown <= 0) {
                ...
                gameBorder.setCenter(new Location(FloorIsLava.getVoidWorld(), (gamePlot.plotStart.getX() + (FloorIsLava.getGamePlotDivider().plotSize / 2)), 120, gamePlot.plotStart.getZ() + ((double) FloorIsLava.getGamePlotDivider().plotSize / 2)));
                ...
                runBackMusic();
                inviteLobbyManager.closeLobby(inviteLobbyManager.getLobbyFromOwner(owner));
                beginLavaTimer();
                beginEventTimer();
                task.cancel();
            }
    ...
    public void endGame() {
        ...
        Bukkit.getScheduler().runTaskLater(FloorIsLava.getInstance(), (task) -> {
            while (this.players.size() > 0) {
                remove(this.players.get(0), false);
            }
